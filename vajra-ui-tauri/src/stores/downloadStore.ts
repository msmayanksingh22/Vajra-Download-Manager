...
export const useDownloadStore = create<DownloadStore>((set, get) => ({
  ...
  addOrUpdateDownload: (download) => set((state) => {
    ...
    if (existing) {
      ...
      if (download.status === 'idle' && (existing.status === 'paused' || existing.status === 'failed')) {
  ...
  batchUpdateDownloads: (updates) => set((state) => {
  ...
  setDownloads: (downloadsList) => set((state) => {
    ...
    for (const d of downloadsList) {
      ...
      if (existing) {
        // Protect status: do not let an 'idle' status override a 'paused' or 'failed' status
        if (d.status === 'idle' && (existing.status === 'paused' || existing.status === 'failed')) {
          merged.status = existing.status;
        }
