...
export default function BatchRenameDialog({ items, onClose }: BatchRenameDialogProps) {
  ...
  return (
    <div className="dialog-overlay" style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
      <div style={{ background: 'var(--color-surface)', padding: 24, borderRadius: 8, width: 400 }}>
        ...
        <input 
          ...
          style={{ width: '100%', padding: '8px', background: 'var(--color-surface-raised)', border: '1px solid var(--color-border)', color: '#fff', borderRadius: 4, marginBottom: 16 }}
        />
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
          <button onClick={onClose} style={{ padding: '6px 12px', background: 'transparent', border: '1px solid var(--color-border)', color: '#fff', borderRadius: 4, cursor: 'pointer' }}>Cancel</button>
          <button onClick={handleApply} style={{ padding: '6px 12px', background: 'var(--color-brand)', border: 'none', color: '#000', borderRadius: 4, cursor: 'pointer', fontWeight: 600 }}>Rename {items.length} items</button>
        </div>
      </div>
    </div>
  );
