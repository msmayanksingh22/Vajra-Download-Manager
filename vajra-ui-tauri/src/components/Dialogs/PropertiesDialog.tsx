...
export default function PropertiesDialog({ item, onClose }: any) {
  ...
  const [speedLimit, setSpeedLimit] = useState(
    item.speed_limit_bps ? Math.floor(item.speed_limit_bps / 1024) : '',
  );
  ...
  const SectionHead = ({ icon: Icon, label }: { icon: React.ElementType; label: string }) => (
    ...
    >
      ...
      <span
        style={{ fontSize: 'var(--text-sm-size)', fontWeight: 700, color: 'var(--color-text-1)' }}
      >
        {label}
      </span>
    </div>
        ...
        <div className="dialog-header">
          ...
          >
            ...
            <div className="min-w-0">
              ...
              <div style={{ fontSize: 'var(--text-xs-size)', color: 'var(--color-text-4)' }}>
                ID: {item.id}
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="form-group">
              ...
              <input
                type="text"
                className="input-field"
                placeholder="e.g. video, work, urgent"
                value={tagsInput}
              ...
              <div className="flex gap-2">
                {['high', 'normal', 'low'].map((p) => (
                  <button
                    ...
                    className={cn(
                      'flex-1 py-1.5 rounded border text-xs font-bold uppercase tracking-wider transition-colors',
              ...
              <div className="flex items-center gap-2">
                <div
                  ...
                  style={{
                    ...
                    animation: item.status === 'downloading' ? 'pulse 2s infinite' : 'none',
                  }}
                />
                <span
                  style={{
