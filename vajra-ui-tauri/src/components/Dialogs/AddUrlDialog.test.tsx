...
describe('AddUrlDialog', () => {
  ...
  it('renders correctly and sets initial URL', () => {
    render(
      <AddUrlDialog initialUrl="http://example.com/file.zip" onOk={vi.fn()} onClose={vi.fn()} />,
    );
  ...
  it('triggers inspect when URL is entered', async () => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    (api.inspect as any).mockResolvedValue({
      filename: 'test.zip',
      size: 1024,
  ...
  it('auto-fills credentials if domain matches local storage', async () => {
    localStorage.setItem(
      'vajra_site_logins',
      JSON.stringify([{ host: 'example.com', user: 'admin', pass: 'secret' }]),
    );

    render(
      <AddUrlDialog initialUrl="http://example.com/secret.zip" onOk={vi.fn()} onClose={vi.fn()} />,
    );
