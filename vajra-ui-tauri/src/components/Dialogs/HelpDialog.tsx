    ...
    >
      ...
      >
        ...
        <div className="dialog-header">
          ...
          <button ref={closeBtnRef} className="btn-icon" onClick={onClose} title="Close" aria-label="Close help dialog">
            <X size={15} />
          </button>
        </div>
                    ...
                    <div style={{ display: 'flex', gap: 3, flexShrink: 0, alignItems: 'center' }}>
                      {keys.map((k, i) => (
                        <React.Fragment key={k}>
                          {i > 0 && (
                            <span style={{ fontSize: 'var(--text-xs-size)', color: 'var(--color-text-4)' }}>+</span>
                          )}
                          <Kbd>{k}</Kbd>
                        </React.Fragment>
                      ))}
