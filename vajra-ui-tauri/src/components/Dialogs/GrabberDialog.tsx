...
export function GrabberDialog({ onClose }: { onClose: () => void }) {
  ...
  function handleStart(e: React.FormEvent) {
    ...
    es.onmessage = (ev) => {
      try {
        ...
        setResults((p) => (p.some((r) => r.url === d.url) ? p : [...p, d]));
      } catch {
        /* ignore */
      }
    };
    es.onerror = () => {
      ...
      setResults((prev) => {
        if (prev.length === 0 && hadResults) {
          setGrabError(
            'Could not connect to daemon or no resources found. Make sure Vajra is running.',
          );
        }
        return prev;
      });
  ...
  const selectAll = () =>
    setSelected(
      selected.size === filtered.length ? new Set() : new Set(filtered.map((r) => r.url)),
    );

  async function downloadSelected() {
    for (const r of results.filter((r) => selected.has(r.url))) {
      try {
        await api.add({ url: r.url });
      } catch {
        /* ignore */
      }
    }
  ...
  const tabRow = (items: string[], activeKey: string, setKey: (k: string) => void) => (
    <div
      className="flex gap-5 px-4 pt-2 shrink-0 drag-region"
      style={{
  ...
  return (
    <div className="dialog-overlay" onClick={onClose}>
      <div
        ...
        style={{ width: 780, height: '78vh', maxHeight: 620 }}
        onClick={(e) => e.stopPropagation()}
        role="dialog"
        aria-modal="true"
        aria-labelledby="grabber-dialog-title"
      >
        {/* Header */}
        ...
        >
          <form onSubmit={handleStart} className="flex gap-2">
            <input
              ...
              placeholder="Root URL to grab resources fromâ€¦"
              value={url}
        ...
        {/* Filter tabs */}
        {results.length > 0 &&
          tabRow(['all', 'video', 'audio', 'image', 'document', 'page'], filter, setFilter)}
        ...
        <div className="flex-1 overflow-y-auto" style={{ backgroundColor: 'var(--color-surface)' }}>
          {filtered.length === 0 ? (
              ...
              {grabError ? (
                <>
                  ...
                  <button
                    ...
                    style={{ fontSize: 'var(--text-xs-size)', marginTop: 4 }}
                ...
                <>
                  ...
                  />
                  <span style={{ fontSize: 'var(--text-sm-size)', fontWeight: 600 }}>
                    Grabbing…
                  </span>
                </>
              ) : (
                <span style={{ fontSize: 'var(--text-sm-size)' }}>
                  No results. Enter a URL and start the grabber.
                </span>
              )}
            </div>
          ) : (
            <div className="flex flex-col">
              {filtered.map((r) => {
                const def = TYPE_ICONS[r.resource_type] || {
                  icon: Link,
                    ...
                    <div className="flex-1 min-w-0 flex items-center justify-between gap-4">
                      <span
                        ...
                        style={{
                          fontSize: 'var(--text-sm-size)',
                          fontWeight: 600,
                          color: isSel ? 'var(--color-brand)' : 'var(--color-text-1)',
                        }}
                        title={r.name}
                      >
                        {r.name}
        ...
        <div className="dialog-footer justify-between">
          ...
          <div className="flex gap-2">
            <button className="btn-secondary" onClick={selectAll} disabled={filtered.length === 0}>
              Select All
            </button>
            <button
