...
export default function ProgressWindow({ downloadId }: { downloadId: string }) {
  ...
  const updateItemFromPayload = (payload: any) => {
    ...
    setItem((prev: any) => {
      ...
      if (override && now - override.timestamp < 1200) {
        ...
        if (
          ...
          (override.status === 'connecting' && payload.status === 'downloading')
        ) {
      ...
      if (nextStatus === 'completed' && prevStatus && prevStatus !== 'completed') {
        const storedComplete = localStorage.getItem(
          `vajra_show_complete_${payload.download_id || payload.id}`,
        );
      ...
      return {
        ...
        downloaded_bytes:
          payload.downloaded_bytes ?? payload.bytes_done ?? prev?.downloaded_bytes ?? 0,
        ...
        eta_seconds:
          nextStatus === 'paused' ? null : (payload.eta_seconds ?? prev?.eta_seconds ?? null),
    ...
    if (payload.speed_bps !== undefined) {
      if (currentStatus === 'downloading') {
        ...
        setSpeedHistory((prev) =>
          prev.length > 0 && prev[prev.length - 1] === 0 ? prev : [...prev, 0].slice(-50),
        );
      }
    }
  };
  ...
  useEffect(() => {
    ...
    const startPolling = (intervalMs: number) => {
      ...
      const poll = () => {
        api
          .get(downloadId)
          .then((data) => {
            if (data) updateItemFromPayload(data);
          })
          .catch(() => {});
      };
      poll();
      ...
      listen('vajra-event', (event: any) => {
        ...
        if (ev.event === 'batch_progress') {
          ...
          if (downloads && Array.isArray(downloads)) {
            const myDownload = downloads.find(
              (d: any) => (d.download_id || d.id)?.toLowerCase() === downloadId.toLowerCase(),
            );
            if (myDownload) {
              setSseRetries(0);
        ...
        if (ev.event === 'progress') {
          ...
          if (override && now - override.timestamp < 1200) {
            ...
            if (
              ev.status === override.status ||
              (override.status === 'connecting' && ev.status === 'downloading')
            ) {
              lastOverride.current = null;
            }
          }
          setItem((prev: any) => {
            ...
            if (status === 'completed' && base.status !== 'completed') {
              const storedComplete = localStorage.getItem(
                `vajra_show_complete_${ev.id || downloadId}`,
              );
              if (storedComplete === 'true') {
                (async () => {
                  try {
    ...
    if (connectionMode === 'sse') {
      ...
      return () => {
        ...
        clearInterval(sseCheckIntervalId);
      };
    }
  ...
  const handlePause = async () => {
    ...
    try {
      await api.patch(downloadId, { action: 'pause' });
    } catch (e) {
      console.error(e);
    }
  };
  ...
  const handleOpenFolder = async () => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    try {
      if (item && (item as any).output_path) {
        const { invoke } = await import('@tauri-apps/api/core');
        await invoke('show_in_explorer', { path: (item as any).output_path });
      }
    } catch (e) {
  ...
  const handleOpenFile = async () => {
    ...
    try {
      if (item && (item as any).output_path) {
        const { invoke } = await import('@tauri-apps/api/core');
        await invoke('open_file_path', { path: (item as any).output_path });
      }
    } catch (e) {
  ...
  const getSegmentColors = (status: string): { track: string; fill: string } => {
    switch (status) {
      case 'completed':
        return { track: 'var(--color-success-dim)', fill: 'var(--color-success)' };
      case 'downloading':
        return { track: 'var(--color-brand-dim)', fill: 'var(--color-brand)' };
      case 'paused':
        return { track: 'var(--color-warning-dim)', fill: 'var(--color-warning)' };
      case 'failed':
        return { track: 'var(--color-error-dim)', fill: 'var(--color-error)' };
      default:
        return { track: 'var(--color-surface-elevated)', fill: 'var(--color-text-4)' };
    }
  };
    ...
    >
      ...
      <div className="drag-region window-titlebar">
        ...
        >
          <Activity
            size={14}
            style={{
              color: currentItem.status === 'downloading' || retryAttempt > 0 ? S.brand : S.t4,
            }}
          />
          {retryAttempt > 0
      ...
      >
        ...
        >
          ...
          >
            <span style={{ color: S.brand, fontWeight: 600 }}>URL: </span>
            {currentItem.url}
          </div>
          {currentItem.output_path && (
            ...
            >
              <span style={{ color: S.t3, fontWeight: 600 }}>DIR: </span>
              {currentItem.output_path}
            </div>
          )}
          {currentItem.expected_hash && (
            ...
            >
              <ShieldCheck
                ...
                style={{
                  color:
                    currentItem.hash_result === 'Matched'
                      ? S.success
                      : currentItem.hash_result === 'Mismatched'
                        ? S.error
                        : S.t4,
                }}
              />
          ...
          {[
            {
              ...
              value: (
                <>
                  {fmtBytes(Math.min(dlBytes, totBytes || dlBytes))}{' '}
                  <span style={{ color: S.t4, fontSize: 'var(--text-xs-size)' }}>
                    / {totBytes ? fmtBytes(totBytes) : '?'}
                  </span>
                </>
              ),
            },
            ...
            },
            {
              label: 'Threads',
              value:
                currentItem.status === 'downloading' && segments.length > 0
                  ? `${segments.length} Active`
                  : '—',
            },
          ].map(({ label, value }) => (
            <div key={label} style={{ display: 'flex', flexDirection: 'column' }}>
              <span style={{ fontSize: 'var(--text-xs-size)', color: S.t3, fontWeight: 500 }}>
                {label}
              </span>
              <span
                style={{
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  if (!isNaN(kbps) && kbps > 0) {
                    setEnableLimit(true);
                    api.patch(downloadId, { speed_limit_bps: kbps * 1024 }).catch(console.error);
                  } else api.patch(downloadId, { speed_limit_bps: 0 }).catch(console.error);
                }}
              />
              <span style={{ fontSize: 'var(--text-xs-size)', color: S.t4, whiteSpace: 'nowrap' }}>
                KB/s
              </span>
            </div>
          </div>
              ...
              {[
                {
                  ...
                  onChange: (v: boolean) => {
                    setShowCompleteDialog(v);
                    localStorage.setItem(`vajra_show_complete_${downloadId}`, String(v));
                  },
                },
                {
                  label: 'Shutdown PC',
                  ...
                  onChange: (v: boolean) => {
                    setShutdownAfterComplete(v);
                    localStorage.setItem(`vajra_shutdown_${downloadId}`, String(v));
                  },
                },
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4, flexShrink: 0 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
            <span style={{ fontSize: 'var(--text-xs-size)', fontWeight: 600, color: S.t3 }}>
              Progress
            </span>
            <span
              style={{
                fontWeight: 700,
                fontSize: 'var(--text-sm-size)',
                color: currentItem.status === 'failed' ? S.error : S.brand,
              }}
            >
              {pct.toFixed(2)}%
            </span>
        ...
        {currentItem.status === 'failed' ? (
          ...
          >
            ...
            <div>
              <h4
                style={{ margin: 0, fontSize: 'var(--text-md-size)', fontWeight: 600, color: S.t1 }}
              >
          ...
          <>
                ...
                >
                  <defs>
                    <linearGradient id="chartGrad" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="0%" stopColor={S.brand} stopOpacity={0.3} />
                      <stop offset="100%" stopColor={S.brand} stopOpacity={0} />
                    </linearGradient>
                  </defs>
                  {/* eslint-disable-next-line @typescript-eslint/no-explicit-any */}
                  <path
                    d={`M 0,30 L ${speedHistory
                      .map((s, i) => {
                        const x = (i / (speedHistory.length - 1)) * 500;
                        const mx = Math.max(...(speedHistory as any), 1024 * 1024);
                        const y = 30 - ((s as any) / mx) * 30 * 0.8 - 1;
                        return `${x},${y}`;
                      })
                      .join(' ')} L 500,30 Z`}
                    fill="url(#chartGrad)"
            ...
            {currentItem.status !== 'completed' && segments.length > 0 && (
              <div
                style={{ display: 'flex', flexDirection: 'column', gap: 4, flex: 1, minHeight: 0 }}
              >
                <div
                  style={{
                    display: 'flex',
                ...
                >
                  ...
                  {segments.map((seg: any, idx: number) => {
                    const wp =
                      fileTotal > 0
                        ? (seg.allocated_bytes / fileTotal) * 100
                        : 100 / segments.length;
                    const fp =
                      seg.allocated_bytes > 0
                        ? Math.min(100, Math.max(0, (seg.bytes_done / seg.allocated_bytes) * 100))
                        : 0;
                      ...
                      <tbody>
                        ...
                        {segments.map((seg: any, idx: number) => (
                          <tr
                            ...
                            onMouseEnter={(e) =>
                              (e.currentTarget.style.backgroundColor = S.elevated)
                            }
                            onMouseLeave={(e) =>
                          ...
                          >
                            ...
                            <td style={{ padding: '5px 10px', color: S.t3, whiteSpace: 'nowrap' }}>
                              {fmtBytes(seg.bytes_done)} /{' '}
                              {seg.allocated_bytes > 0 ? fmtBytes(seg.allocated_bytes) : '???'}
                            </td>
                            <td
                              style={{
                                padding: '5px 10px',
                                fontWeight: 600,
                                color: segStatusColor(seg.status),
                                textAlign: 'right',
                                whiteSpace: 'nowrap',
                              }}
                            >
                              {(seg.status || 'unknown').toUpperCase()}
                            </td>
                          </tr>
                        ))}
                      </tbody>
        ...
        {currentItem.status === 'completed' && (
          ...
          >
            ...
            <div style={{ display: 'flex', gap: 6 }}>
              <button className="btn-secondary flex items-center gap-2" onClick={handleOpenFolder}>
                <Folder size={13} /> Folder
              </button>
              <button className="btn-primary flex items-center gap-2" onClick={handleOpenFile}>
                <FileIcon size={13} /> Open
              </button>
            </div>
          </div>
        )}
        ...
        <div className="flex gap-2 no-drag">
          ...
          {(currentItem.status === 'paused' || currentItem.status === 'failed') && (
            <button
              ...
              onClick={handleResume}
              aria-label={
                currentItem.status === 'failed' ? 'Retry failed download' : 'Resume download'
              }
            >
          ...
          {currentItem.status !== 'completed' && (
            <button className="btn-danger flex items-center gap-2" onClick={handleCancel}>
              <Square size={13} /> Stop
            </button>
          )}
        </div>
