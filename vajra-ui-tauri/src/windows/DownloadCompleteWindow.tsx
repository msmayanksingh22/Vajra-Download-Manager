...
export default function DownloadCompleteWindow({ downloadId }: any) {
  ...
  const openFolder = async () => {
    try {
      await invoke('show_in_explorer', { path: download.output_path });
    } catch {
      /* ignore */
    }
  };
  const openFile = async () => {
    try {
      await invoke('open_file_path', { path: download.output_path });
    } catch {
      /* ignore */
    }
    close();
  ...
  return (
    ...
    >
          ...
          <div style={{ minWidth: 0, flex: 1 }}>
            ...
            >
              ...
              {download.hash_result ? (
                <span
                  className={download.hash_result.matched ? 'text-success' : 'text-error'}
          ...
          <InfoCard label="Save Path">
            <div className="flex items-center gap-2 no-drag">
              ...
              <button
                ...
                onClick={() => copy(download.output_path, 'path')}
                title="Copy path"
              >
                {copiedField === 'path' ? (
      ...
      >
        ...
        <button className="btn-secondary flex items-center gap-2" onClick={openFolder}>
          <FolderOpen size={14} /> Show in Folder
        </button>
        <button className="btn-primary flex items-center gap-2" onClick={openFile}>
