...
impl MirrorManager {
    ...
    pub fn rank_mirrors(&self, results: &[(String, MirrorHealth)]) -> Vec<String> {
        ...
        ranked.sort_by(|a, b| {
            let a_score = (a.1.supports_ranges as u8, a.1.speed_bps, 1.0 / (a.1.latency_ms as f64).max(1.0));
            let b_score = (b.1.supports_ranges as u8, b.1.speed_bps, 1.0 / (b.1.latency_ms as f64).max(1.0));
