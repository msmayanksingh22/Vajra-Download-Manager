...
use vajra_engine::allocator;
use vajra_engine::writer::{start_disk_writer, DataFrame};
use bytes::Bytes;
...
fn bench_allocate_file_space(c: &mut Criterion) {
    ...
    for (size, name) in sizes.iter() {
        group.bench_with_input(BenchmarkId::new("allocate_file_space", name), size, |b, &s| {
...
fn bench_chunk_merge(c: &mut Criterion) {
    ...
    group.bench_function("start_disk_writer_100MB", |b| {
        b.to_async(&rt).iter(|| async {
            ...
            allocator::allocate_file_space(&file_path, total_size).await.unwrap();
            ...
            let writer_fut = tokio::spawn(async move {
                start_disk_writer(&file_path_clone, rx).await.unwrap()
            });
